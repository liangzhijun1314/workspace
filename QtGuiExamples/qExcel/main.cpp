...
int main(int argc, char *argv[])
{
    ...
    qDebug()<<"Current Dir path " << QDir::currentPath();
        QExcel j(QCoreApplication::applicationDirPath() + "/test.xlsx");
    //取得工作表数量
    ...
    qDebug()<<"topLeftRow = "<<topLeftRow;
    qDebug()<<"topLeftColumn = "<<topLeftColumn;
    qDebug()<<"bottomRightRow = "<<bottomRightRow;
    qDebug()<<"bottomRightColumn = "<<bottomRightColumn;
    //读值
    int i,k;
    for(i = topLeftRow+2; i < topLeftRow+3; i++){
        for(k = topLeftColumn; k < bottomRightColumn+1; k++){

            qDebug()<<"value = "<<j.getCellValue(i, k).toString();

        }
    }
//    qDebug()<<"value = "<<j.getCellValue(10, 40).toString();
